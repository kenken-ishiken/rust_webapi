...
mod tests {
    ...
    async fn test_mock_validate_circular_reference() {
        ...
        mock_repo
            ...
            .with(
                predicate::eq("cat_parent"),
                predicate::eq(Some("cat_child".to_string())),
            )
            .return_once(|_, _| {
                Err(CategoryError::CircularReference(
                    "循環参照が検出されました".to_string(),
                ))
            });

        let result = mock_repo
            .validate_circular_reference("cat_parent", Some("cat_child".to_string()))
            .await;
        assert!(result.is_err());

        match result {
            Err(CategoryError::CircularReference(msg)) => {
                assert_eq!(msg, "循環参照が検出されました")
            }
