...
async fn test_e2e_health_check() {
    ...
    let app = test::init_service(
        create_test_app(item_service.clone(), user_service.clone())
    ).await;
...
async fn test_e2e_items_crud_workflow() {
    ...
    if resp.status() != 201 {
        ...
        let body = test::read_body(resp).await;
        println!("Expected 201, got {} with body: {:?}", status, std::str::from_utf8(&body));
    ...
    assert_eq!(updated_item["description"], "Updated description via E2E test");
...
async fn test_e2e_error_handling() {
    ...
    let req = test::TestRequest::get().uri("/api/items/99999").to_request();
    let resp = test::call_service(&app, req).await;
...
async fn test_e2e_concurrent_operations() {
    ...
    let create_tasks: Vec<_> = (1..=5).map(|i| {
        let app = &app;
        async move {
            let create_request = json!({
    ...
    println!("✅ E2E Concurrent operations test passed - {} items created", items_array.len());
}