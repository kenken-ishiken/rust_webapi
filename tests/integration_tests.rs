...
integration_test!(test_user_crud_operations, |factory: TestRepositoryFactory| async move {
...
integration_test!(test_batch_operations, |factory: TestRepositoryFactory| async move {
    let repo = factory.create_item_repository();
    ...
    let deleted_ids = repo.batch_delete(ids_to_delete.clone(), false).await.unwrap();
    ...
    let physically_deleted_ids = repo.batch_delete(remaining_ids.clone(), true).await.unwrap();
...
in_memory_only_test!(test_in_memory_specific_features, |factory: TestRepositoryFactory| async move {
