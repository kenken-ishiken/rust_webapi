...
async fn test_item_deletion_strategy_logical_deletion() {
    ...
    let created = repository.create(item).await.expect("Failed to create item");
    ...
    // アイテムが論理削除されていることを確認
    let deleted_items = repository.find_deleted().await.expect("Find deleted should not fail");
...
async fn test_item_deletion_strategy_physical_deletion() {
    ...
    repository.create(item).await.expect("Failed to create item");
    ...
    // アイテムが物理削除されていることを確認
    let found = repository.find_by_id(1).await.expect("Find should not fail");
...
async fn test_item_deletion_strategy_performance() {
    ...
    for i in 1..=10 {
        ...
        let created = repository.create(item).await.expect("Failed to create test item");
    ...
    assert!(duration.as_secs() < 1, "Deletion performance too slow: {:?}", duration);
...
async fn test_deletion_strategy_error_consistency() {
    ...
    assert!(matches!(logical_result.unwrap_err(), DeletionError::NotFound(_)));
    assert!(matches!(physical_result.unwrap_err(), DeletionError::NotFound(_)));
    assert!(matches!(restore_result.unwrap_err(), DeletionError::NotFound(_)));
}
